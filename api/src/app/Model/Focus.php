...
class Focus extends NotORM
{
  ...
  /**
      * 删除
    */
  public function deleteOne($uid, $fid)
  {
    return $this->getORM()->where("UserId", $uid)
      ->where("FanId", $fid)->delete();
  }
  /**
   * 查看某人是否关注了某人
   */
  public function judgeUserFocusUser($uid,$foid)
  {
    $re=$this->getORM()->where("UserId", $foid)
      ->where("FanId", $uid)->count();
