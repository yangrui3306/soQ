...
class Mistake extends NotORM
{
  ...
  /**
     * 根据题目Id查找所有错题
		 * @param qid 题目id
     */
  public function getMistakeByQId($qid, $num = 0)
  {
    $re = $this->getORM()
      ->select('*')
      ->where('QuestionId', $qid)->order("CollectNumber,LikeNumber DESC");
    if ($num > 0) return $re->limit($num)->fetchAll();
  ...
  /**
			...
			* @param uid 用户id
      */
  public function getMistakeByUId($uid, $num = 0)
  {
    $re = $this->getORM()
      ->select('*')
      ->where('UserId', $uid)
  ...
  */
   ...
   * @return 返回受影响的行数
   */
  public function likeMistake($misid)
  {
    return $this->getORM()->
    where('Id',$misid)
    ->update(array('LikeNumber'=>new \NotORM_Literal("LikeNumber + 1")));
  }
  /**
   * 收藏
   * @param 错题Id
   * @return 返回受影响的行数
   */
  public function collectionMistake($misid)
  {
    return $this->getORM()->
    where('Id',$misid)
    ->update(array('CollectNumber'=>new \NotORM_Literal("CollectNumber + 1")));
  }
}
